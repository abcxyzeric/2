...
export const gameplayAiService = {
  ...
  async generateStoryTurn(
    ...
    try {
        ...
        // Find relevant memories from the distant past - ISOLATED BY WORLD ID
        const similarVectors = await vectorService.searchSimilarVectors(input, worldData.id, 5);
        ...
        // Task 3.3 Step 4: Save Vectors Async (Fire and forget) - WITH WORLD ID
        (async () => {
             const userMsgId = `msg-${Date.now()}-user`;
             const aiMsgId = `msg-${Date.now() + 1}-model`;
             await vectorService.saveVector(userMsgId, input, 'user', worldData.id);
             if (fullResponse) {
                 await vectorService.saveVector(aiMsgId, fullResponse, 'model', worldData.id);
             }
        })();
  ...
  async *generateStoryTurnStream(
    ...
    try {
        ...
        // Task 3.3 Step 1: Vector Search (RAG) - ISOLATED BY WORLD ID
        const similarVectors = await vectorService.searchSimilarVectors(input, worldData.id, 5);
        const relevantMemories = similarVectors
            .map(v => `[${new Date(v.timestamp).toLocaleString()}] ${v.role === 'user' ? 'User' : 'AI'}: ${v.text}`)
            .join('\n\n');
        ...
        // Task 3.3 Step 4: Save Vectors Async after stream completes - WITH WORLD ID
        (async () => {
             const userMsgId = `msg-${Date.now()}-user`;
             const aiMsgId = `msg-${Date.now() + 1}-model`;
             await vectorService.saveVector(userMsgId, input, 'user', worldData.id);
             if (accumulatedFullText) {
                 await vectorService.saveVector(aiMsgId, accumulatedFullText, 'model', worldData.id);
             }
        })();
