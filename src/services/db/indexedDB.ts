...
export interface VectorData {
  id: string;
  worldId: string; // ID của thế giới để cô lập dữ liệu
  text: string;
  embedding: number[];
  timestamp: number;
  role: 'user' | 'model';
...
interface RPGDatabase extends DBSchema {
  ...
  vectors: {
    key: string;
    value: VectorData;
    indexes: { 'worldId': string }; // Index for faster filtering
  };
}

const DB_NAME = 'aetheria-rpg-db';
const DB_VERSION = 3; // Upgraded version for Isolation
    ...
    this.dbPromise = openDB<RPGDatabase>(DB_NAME, DB_VERSION, {
      upgrade(db, oldVersion, newVersion, transaction) {
        ...
        if (!db.objectStoreNames.contains('vectors')) {
          ...
          vectorStore.createIndex('worldId', 'worldId'); // Index for isolation
        } else {
          // Upgrade existing store if needed (v2 -> v3)
          const vectorStore = transaction.objectStore('vectors');
          if (!vectorStore.indexNames.contains('worldId')) {
             vectorStore.createIndex('worldId', 'worldId');
          }
        }
      },
    });
