...
const WorldCreationScreen: React.FC<WorldCreationProps> = ({ onNavigate, onGameStart, initialData }) => {
  ...
  const handleExportWorld = () => {
    // Generate a temporary ID for export if needed, or leave blank if it's just a setup template
  ...
  const handleStartGame = () => {
     // Create a unique ID for this new game campaign
     const newWorldId = `campaign-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;

     const worldData: WorldData = {
        id: newWorldId, // Assign Unique ID
        player: state.player,
        world: state.world,
        config: state.config,
        entities: state.entities
