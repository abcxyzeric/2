  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (activeWorld) {
          ...
          // Ensure World ID exists (Migration for legacy saves)
          const currentWorldId = worldDataWithState.id || `legacy-${Date.now()}`;
          if (!worldDataWithState.id) {
              worldDataWithState.id = currentWorldId;
              console.log("Assigned legacy ID to world:", currentWorldId);
          }
          ...
          if (worldDataWithState.savedState) {
              ...
              // Task 3.4: Background vectorization of existing history - Pass worldId
              setTimeout(() => {
                  vectorService.vectorizeAllHistory(worldDataWithState.savedState!.history, currentWorldId);
              }, 1000);
