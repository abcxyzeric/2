...
const TawaPresetManager: React.FC<TawaPresetManagerProps> = ({ onConfigChange }) => {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        // Merge with default to ensure structure validity if version changes
        const merged = { ...DEFAULT_PRESET_CONFIG, ...parsed };
        setConfig(merged);
        onConfigChange(merged);
      } catch (e) {
        console.error("Failed to parse saved Tawa Config", e);
        setConfig(DEFAULT_PRESET_CONFIG);
        onConfigChange(DEFAULT_PRESET_CONFIG);
      }
    } else {
      setConfig(DEFAULT_PRESET_CONFIG);
      onConfigChange(DEFAULT_PRESET_CONFIG);
  ...
  // 2. Persist: Save to LocalStorage whenever config changes
  useEffect(() => {
  ...
  const handleReset = () => {
    if (window.confirm("Bạn có chắc chắn muốn khôi phục toàn bộ Preset về mặc định? Mọi tùy chỉnh sẽ bị mất.")) {
      try {
  ...
  const renderEditor = (id: string, label: string, content: string) => (
      ...
      >
          <div className="flex justify-between items-center mb-1">
              <span className="text-[10px] text-slate-500 uppercase font-bold">Editing: {label}</span>
              <button onClick={() => setEditingId(null)} className="text-[10px] text-green-400 flex items-center gap-1 hover:underline">
                  <Check size={12}/> Done
              </button>
          </div>
          <textarea 
  ...
  return (
    <>
        ...
        <button 
            ...
            className="w-full p-3 flex items-center justify-between text-left hover:bg-slate-700/50 transition-colors group rounded-lg border border-slate-700 bg-slate-800/30"
        >
            <div className="flex items-center gap-2 text-sm font-bold text-slate-300 group-hover:text-mystic-accent transition-colors">
                <Settings2 size={16} />
                Preset
            </div>
            <div className="text-[10px] text-slate-500 bg-slate-800 px-2 py-0.5 rounded border border-slate-700">
            ...
            {isOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
                    <MotionDiv 
                        ...
                        className="bg-mystic-900 border border-slate-700 w-full max-w-3xl rounded-xl shadow-2xl flex flex-col max-h-[85vh] overflow-hidden"
                    >
                        {/* Modal Header */}
                            ...
                            <div className="bg-slate-800/30 p-4 rounded-lg border border-slate-700/50">
                                <div className="flex justify-between items-center mb-2">
                                    ...
                                    <button 
                                        ...
                                        className={`flex items-center gap-1 px-2 py-1 rounded text-xs transition-colors border ${editingId === config.cot.id ? 'bg-slate-700 border-mystic-accent text-mystic-accent' : 'border-slate-600 text-slate-400 hover:text-white'}`}
                                    >
                                        ...
                                        <div className="flex justify-between items-start">
                                            <div className="flex-1 mr-4">
                                                <div className="flex items-center gap-2 mb-1">
                                                     <span className={`text-sm font-medium ${mod.isActive ? 'text-slate-200' : 'text-slate-500'}`}>
                                                        {mod.label}
                                                    </span>
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    <button 
                                                        ...
                                                        className={`text-xs flex items-center gap-1 hover:underline ${editingId === mod.id ? 'text-mystic-accent' : 'text-slate-500'}`}
                                                    >
                                            ...
                                            <button 
                                                ...
                                                className={`${mod.isActive ? 'text-green-400' : 'text-slate-600'} hover:scale-110 transition-transform`}
                        ...
                        <div className="p-4 border-t border-slate-800 bg-slate-900/50 flex justify-between items-center shrink-0">
                            <button 
                                onClick={handleReset}
                                className="flex items-center gap-1.5 text-xs text-red-400 hover:text-red-300 hover:bg-red-900/20 px-3 py-2 rounded transition-colors"
                            >
                            ...
                            <button 
                                ...
                                className="px-6 py-2 bg-mystic-800 border border-mystic-accent/30 text-mystic-accent hover:bg-mystic-accent/10 rounded-lg text-sm font-bold transition-all"
                            >
                                Đóng
                            </button>
                        </div>
