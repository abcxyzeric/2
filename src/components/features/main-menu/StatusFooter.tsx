...
const StatusFooter: React.FC = () => {
  ...
  return (
    <footer className="w-full py-3 px-4 md:px-6 border-t border-slate-800/50 bg-black/60 backdrop-blur-md flex justify-between items-center text-[10px] md:text-xs text-slate-500 font-mono z-50 shrink-0">
      <div className="flex items-center gap-3 md:gap-4">
        <div className="flex items-center gap-2 group cursor-help select-none" title={`Trạng thái DB: ${getStatusText()}`}>
          <Database size={12} className="md:w-3.5 md:h-3.5 text-slate-400" />
